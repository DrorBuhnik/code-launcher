...
export default class CodeLauncherPrefs extends ExtensionPreferences {
  ...
  async fillPreferencesWindow(window) {
    ...
    const page = new Adw.PreferencesPage({
      title: 'Code Launcher', icon_name: 'applications-development-symbolic',
    });

    const scanGroup = new Adw.PreferencesGroup({
      title: 'Scanning', description: 'Choose the directory to scan for .idea projects.',
    });

    const dirRow = new Adw.ActionRow({
      title: 'Scan directory', subtitle: 'Root folder to scan recursively for .idea folders',
    });
    ...
    const currentDirLabel = new Gtk.Label({
      xalign: 1, selectable: true, ellipsize: 3,
    });
    ...
    chooseBtn.connect('clicked', () => {
      ...
      dialog.select_folder(window, null, (d, res) => {
        try {
          ...
          // cancel is normal
          console.error(`[Code Launcher] select_folder failed: ${e}`);
        }
      });
    });
    ...
    const panelGroup = new Adw.PreferencesGroup({
      title: 'Panel', description: 'Control the order of the indicator on the right side of the top bar.',
    });

    const indexRow = new Adw.SpinRow({
      title: 'Order (index)', subtitle: '0 = first in the right section. Increase to move rightward.', adjustment: new Gtk.Adjustment({
        lower: 0, upper: 50, step_increment: 1, page_increment: 5, value: 0,
      }),
    });
    ...
    const projectsGroup = new Adw.PreferencesGroup({
      title: 'Detected projects', description: 'Toggle projects to show or hide them from the top-bar popup.',
    });

    const toolbarRow = new Adw.ActionRow({
      title: 'Controls', subtitle: 'Scan and manage which projects appear in the popup.',
    });

    const rescanBtn = new Gtk.Button({
      label: 'Rescan', valign: Gtk.Align.CENTER,
    });
    ...
    const clearProjectsList = () => {
      for (const row of projectRows) projectsGroup.remove(row);
    ...
    const rebuildProjectsList = () => {
      ...
      if (!rootPath || rootPath.trim() === '') {
        const empty = new Adw.ActionRow({
          title: 'Set a scan directory first', subtitle: 'Go to "Scanning" above and choose a folder.',
        });
      ...
      if (scannedProjects.length === 0) {
        const empty = new Adw.ActionRow({
          title: 'No projects detected yet', subtitle: 'Click "Rescan" to scan for projects.',
        });
        empty.set_sensitive(false);
        projectsGroup.add(empty);
        projectRows.push(empty);
      ...
      const sorted = [...scannedProjects].sort((a, b) => getProjectDisplayLabel(a).toLowerCase().localeCompare(getProjectDisplayLabel(b).toLowerCase()));

      const ignored = getIgnored();
      for (const p of sorted) {
        const label = getProjectDisplayLabel(p);
        const row = new Adw.ActionRow({
          title: label, subtitle: p, selectable: false,
        });
    ...
    const doRescan = async () => {
      ...
      try {
        ...
        console.error(`[Code Launcher] prefs rescan failed: ${e}`);
        scannedProjects = [];
      }
